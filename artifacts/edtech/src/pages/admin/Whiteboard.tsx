...
import {
  ArrowLeft, Save, Trash2, Download, Undo2, Redo2, ZoomIn, ZoomOut, Grid, Maximize2, Copy,
} from "lucide-react";
...
type PenStyle = "round" | "marker" | "calligraphy" | "pencil";
type EraserMode = "stroke" | "element";
...
const STICKY_COLORS = ["#fef08a", "#86efac", "#93c5fd", "#f9a8d4", "#c4b5fd", "#fdba74"];
...
function getBgFill(bg: BgStyle) { return bg === "dark" || bg === "darkgrid" ? "#1e1e2e" : "#ffffff"; }
...
function drawBackground(ctx: CanvasRenderingContext2D, bg: BgStyle) {
  ...
  if (bg === "grid") {
    ...
    for (let y = 0; y <= CH; y += sp * sq3 / 2) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(CW, y); ctx.stroke(); }
  }
}
...
function hexPath(cx: number, cy: number, r: number): Path2D {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    i === 0 ? p.moveTo(cx + r * Math.cos(a), cy + r * Math.sin(a)) : p.lineTo(cx + r * Math.cos(a), cy + r * Math.sin(a));
  }
  p.closePath(); return p;
}
...
function calloutPath(x: number, y: number, w: number, h: number): Path2D {
  ...
  p.lineTo(x + tw + 20, y + h); p.lineTo(x + tw, y + h + th); p.lineTo(x + tw + 10, y + h);
...
function renderAction(ctx: CanvasRenderingContext2D, action: DrawAction) {
  ...
  const doFill = (path: Path2D) => {
    if (action.fillColor) {
      ctx.save(); ctx.globalAlpha = action.opacity * 0.4; ctx.fillStyle = action.fillColor; ctx.setLineDash([]); ctx.fill(path); ctx.restore();
    }
  };
  ...
  if (action.tool === "pen") {
    ...
    const ps = action.penStyle ?? "round";
    if (ps === "marker") {
      ctx.globalAlpha = 1; ctx.lineCap = "square"; ctx.lineJoin = "miter";
      ctx.lineWidth = action.lineWidth * 3;
      ctx.beginPath(); ctx.moveTo(pts[0].x, pts[0].y);
      for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i].x, pts[i].y);
      ctx.stroke();
    } else if (ps === "pencil") {
      ctx.globalAlpha = action.opacity * 0.7; ctx.lineCap = "round"; ctx.lineWidth = Math.max(1, action.lineWidth * 0.7);
      for (let i = 1; i < pts.length; i++) {
        ctx.beginPath(); ctx.moveTo(pts[i - 1].x + (Math.random() - 0.5) * 1.2, pts[i - 1].y + (Math.random() - 0.5) * 1.2);
        ctx.lineTo(pts[i].x + (Math.random() - 0.5) * 1.2, pts[i].y + (Math.random() - 0.5) * 1.2); ctx.stroke();
      }
    } else if (ps === "calligraphy") {
      ctx.lineCap = "round"; ctx.lineJoin = "round";
      for (let i = 1; i < pts.length; i++) {
        const dx = pts[i].x - pts[i - 1].x, dy = pts[i].y - pts[i - 1].y;
        const angle = Math.atan2(dy, dx);
        const w = action.lineWidth * (1 + Math.abs(Math.cos(angle - Math.PI / 4)) * 2.5);
    ...
    ctx.lineWidth = Math.max(14, action.lineWidth * 3); ctx.lineCap = "square"; ctx.lineJoin = "square";
    ctx.beginPath(); ctx.moveTo(pts[0].x, pts[0].y);
    for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i].x, pts[i].y);
    ctx.stroke();
    ...
    const pts = action.points;
    if (!pts || pts.length < 1) { ctx.restore(); return; }
    ctx.globalCompositeOperation = "destination-out"; ctx.strokeStyle = "rgba(0,0,0,1)"; ctx.lineWidth = action.lineWidth * 6;
    ctx.beginPath(); ctx.moveTo(pts[0].x, pts[0].y);
    for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i].x, pts[i].y);
    ctx.stroke();
    ...
    const angle = Math.atan2(dy, dx); const headLen = Math.max(18, action.lineWidth * 5);
    ctx.beginPath(); ctx.moveTo(action.start.x, action.start.y); ctx.lineTo(action.end.x, action.end.y); ctx.stroke();
    ctx.setLineDash([]);
    const p = new Path2D();
    ...
    p.lineTo(action.end.x - headLen * Math.cos(angle + Math.PI / 6), action.end.y - headLen * Math.sin(angle + Math.PI / 6));
    p.closePath(); ctx.fillStyle = action.color; ctx.globalAlpha = action.opacity; ctx.fill(p);
    ...
    let rx = Math.abs(action.end.x - action.start.x) / 2, ry = Math.abs(action.end.y - action.start.y) / 2;
    ...
    const p = starPath(cxx, cyy, r); doFill(p); ctx.stroke(p);
  } else if (action.tool === "hexagon" && action.start && action.end) {
    const cxx = (action.start.x + action.end.x) / 2, cyy = (action.start.y + action.end.y) / 2;
    const r = Math.min(Math.abs(action.end.x - action.start.x), Math.abs(action.end.y - action.start.y)) / 2;
    const p = hexPath(cxx, cyy, r); doFill(p); ctx.stroke(p);
    ...
    if (w > 20 && h > 20) {
      const p = calloutPath(x, y, w, h); doFill(p); ctx.stroke(p);
      if (action.text) {
        ctx.fillStyle = action.color; ctx.globalAlpha = action.opacity;
        const fs = action.fontSize ?? 14, pad = 12;
        ctx.font = `${action.fontItalic ? "italic" : ""} ${action.fontBold ? "bold" : ""} ${fs}px ${action.fontFamily ?? "Inter, sans-serif"}`.trim();
        ctx.textBaseline = "top";
        const words = (action.text ?? "").split(" "); let line = "", yl = y + pad;
        for (const word of words) {
          const test = line + word + " ";
          if (ctx.measureText(test).width > w - pad * 2 && line) { ctx.fillText(line.trim(), x + pad, yl); line = word + " "; yl += fs + 4; if (yl > y + h - 40) break; }
          else line = test;
        }
    ...
    ctx.font = `${action.fontItalic ? "italic" : ""} ${action.fontBold ? "bold" : ""} ${fs}px ${action.fontFamily ?? "Inter, sans-serif"}`.trim();
    ctx.fillStyle = action.color; ctx.textBaseline = "top"; ctx.setLineDash([]);
    action.text.split("\n").forEach((line, li) => ctx.fillText(line, action.start!.x, action.start!.y + li * (fs + 4)));
  } else if (action.tool === "sticky" && action.start) {
    const sw = 240, sh = 140, pad = 12, sx = action.start.x, sy = action.start.y;
    ctx.shadowColor = "rgba(0,0,0,0.18)"; ctx.shadowBlur = 12; ctx.shadowOffsetX = 3; ctx.shadowOffsetY = 4;
    ctx.fillStyle = action.fillColor ?? "#fef08a"; ctx.setLineDash([]);
    const p = new Path2D(); p.roundRect(sx, sy, sw, sh, 6); ctx.fill(p);
    ctx.shadowColor = "transparent";
    ctx.fillStyle = action.color; const hdrP = new Path2D(); hdrP.roundRect(sx, sy, sw, 24, [6, 6, 0, 0]); ctx.fill(hdrP);
    ctx.fillStyle = action.color === "#1a1a1a" ? "#fff" : "#1a1a1a"; ctx.font = "bold 11px Inter, sans-serif"; ctx.textBaseline = "middle";
    ctx.fillText("📌 Note", sx + pad, sy + 12);
    ctx.fillStyle = "#1a1a1a"; const fs = action.fontSize ?? 13; ctx.font = `${fs}px Inter, sans-serif`; ctx.textBaseline = "top";
    const words = (action.text ?? "").split(" "); let line = "", yl = sy + 32;
    for (const word of words) {
      const test = line + word + " ";
      if (ctx.measureText(test).width > sw - pad * 2 && line) { ctx.fillText(line.trim(), sx + pad, yl); line = word + " "; yl += fs + 4; if (yl > sy + sh - 12) break; }
      else line = test;
    }
    if (line.trim()) ctx.fillText(line.trim(), sx + pad, yl);
  } else if (action.tool === "image" && action.start && action.imageData) {
    const img = new Image(); img.src = action.imageData;
...
function getBBox(action: DrawAction) {
  if (action.tool === "pen" || action.tool === "highlighter" || action.tool === "eraser") {
    const pts = action.points; if (!pts || pts.length === 0) return null;
  ...
  if (action.start && action.end) {
    ...
    return { x, y, w: Math.abs(action.end.x - action.start.x), h: Math.abs(action.end.y - action.start.y) };
  }
  if (action.start) return { x: action.start.x - 5, y: action.start.y - 5, w: 200, h: 40 };
  return null;
}

function hitTest(action: DrawAction, x: number, y: number, pad = 10) {
  const bb = getBBox(action); if (!bb) return false;
  return x >= bb.x - pad && x <= bb.x + bb.w + pad && y >= bb.y - pad && y <= bb.y + bb.h + pad;
}

function snapToGrid(v: number, gs: number) { return Math.round(v / gs) * gs; }
...
export default function AdminWhiteboard() {
  ...
  const renderLaser = useCallback(() => {
    ...
    if (laserRef.current.length > 1) {
      ...
      for (let i = 1; i < pts.length; i++) {
        ...
        ctx.save(); ctx.globalAlpha = (1 - age) * 0.85; ctx.strokeStyle = "#ef4444"; ctx.lineWidth = Math.max(1, 5 * (1 - age)); ctx.lineCap = "round"; ctx.shadowColor = "#ef4444"; ctx.shadowBlur = 10;
        ctx.beginPath(); ctx.moveTo(pts[i - 1].x, pts[i - 1].y); ctx.lineTo(pts[i].x, pts[i].y); ctx.stroke(); ctx.restore();
      }
      const last = pts[pts.length - 1];
      ctx.save(); ctx.fillStyle = "#ef4444"; ctx.shadowColor = "#ff6666"; ctx.shadowBlur = 16; ctx.globalAlpha = 0.95;
      ctx.beginPath(); ctx.arc(last.x, last.y, 5, 0, Math.PI * 2); ctx.fill(); ctx.restore();
    }
    ...
    if (sid) {
      ...
      if (sel) {
        ...
        if (bb) {
          ...
          ctx.fillStyle = "rgba(99,102,241,0.08)"; ctx.fillRect(bb.x - 6, bb.y - 6, bb.w + 12, bb.h + 12);
          ctx.fillStyle = "#6366f1"; ctx.setLineDash([]);
          [[bb.x - 6, bb.y - 6], [bb.x + bb.w + 6, bb.y - 6], [bb.x - 6, bb.y + bb.h + 6], [bb.x + bb.w + 6, bb.y + bb.h + 6]].forEach(([hx, hy]) => ctx.fillRect(hx - 4, hy - 4, 8, 8));
          ctx.restore();
        }
      }
    }
  ...
  const undoAction = useCallback(() => {
    setActions(prev => { if (!prev.length) return prev; const last = prev[prev.length - 1]; setRedoStack(r => [...r, last]); setSaved(false); return prev.slice(0, -1); });
    setSelectedId(null);
  }, []);
  ...
  const redoAction = useCallback(() => {
    setRedoStack(prev => {
      ...
      setSaved(false); return prev.slice(0, -1);
    });
  }, []);
  ...
  const scheduleAutoSave = (acts: DrawAction[]) => {
    ...
    autoSaveRef.current = setTimeout(() => {
      saveWb.mutate({ classId: cid, data: { data: JSON.stringify({ actions: acts, bgStyle }) } },
  ...
  const doSave = (actsOverride?: DrawAction[]) => {
    ...
    if (autoSaveRef.current) clearTimeout(autoSaveRef.current);
    saveWb.mutate({ classId: cid, data: { data: JSON.stringify({ actions: toSave, bgStyle }) } },
  ...
  useEffect(() => {
    const onDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "Delete" || e.code === "Backspace") {
        if (selectedId) { setActions(prev => { const n = prev.filter(a => a.id !== selectedId); scheduleAutoSave(n); return n; }); setSelectedId(null); }
        return;
      }
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        if (e.key === "c" && selectedId) { e.preventDefault(); const sel = actions.find(a => a.id === selectedId); if (sel) setCopyBuffer(sel); return; }
        if (e.key === "v" && copyBuffer) {
          ...
          setActions(prev => { const n = [...prev, newA]; scheduleAutoSave(n); return n; }); setSelectedId(newA.id); return;
        }
      ...
      const t = TOOL_SHORTCUTS[e.key.toLowerCase()]; if (t) setTool(t);
    };
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (e.button === 1 || (spaceDown && tool !== "select")) {
      isPanningRef.current = true; panStartRef.current = { mx: e.clientX, my: e.clientY, px: pan.x, py: pan.y }; return;
    }
    const pos = getCanvasPos(e.clientX, e.clientY);
    if (tool === "eraser" && eraserMode === "element") {
      const hit = [...actions].reverse().find(a => hitTest(a, pos.x, pos.y));
      if (hit) { setActions(prev => { const n = prev.filter(a => a.id !== hit.id); scheduleAutoSave(n); return n; }); }
      return;
    }
    if (tool === "select") {
      const hit = [...actions].reverse().find(a => hitTest(a, pos.x, pos.y));
      if (hit) {
        setSelectedId(hit.id); isDraggingSelectedRef.current = true;
        ...
        setSelectedId(null); isPanningRef.current = true; panStartRef.current = { mx: e.clientX, my: e.clientY, px: pan.x, py: pan.y };
      }
      return;
    }
    if (tool === "image") { pendingImageRef.current = pos; imageInputRef.current?.click(); return; }
    if (tool === "text" || tool === "sticky" || tool === "callout") {
      const canvas = canvasRef.current; if (!canvas) return;
      const rect = canvas.getBoundingClientRect();
      setTextOverlay({ x: pos.x, y: pos.y, cx: e.clientX - rect.left, cy: e.clientY - rect.top, forTool: tool });
      setTextVal(""); setTimeout(() => textRef.current?.focus(), 30); return;
    }
    setIsDrawing(true); setRedoStack([]);
    const act: DrawAction = { id: uid(), tool, color, fillColor: fillEnabled ? fillColor : null, lineWidth, opacity, dashStyle, fontSize, fontFamily, fontBold, fontItalic, penStyle: tool === "pen" ? penStyle : undefined };
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (tool === "laser" && isLaserActiveRef.current) {
      ...
      if (laserAnimRef.current) cancelAnimationFrame(laserAnimRef.current);
      laserAnimRef.current = requestAnimationFrame(renderLaser); return;
    }
    if (isPanningRef.current) {
      const dx = e.clientX - panStartRef.current.mx, dy = e.clientY - panStartRef.current.my;
      setPan({ x: panStartRef.current.px + dx, y: panStartRef.current.py + dy }); return;
    }
    if (isDraggingSelectedRef.current && dragStartRef.current && selectedId) {
      const orig = dragStartRef.current.origAction;
      const dx = pos.x - dragStartRef.current.mx, dy = pos.y - dragStartRef.current.my;
      setActions(prev => prev.map(a => a.id !== selectedId ? a : {
        ...a, start: orig.start ? { x: orig.start.x + dx, y: orig.start.y + dy } : undefined,
    ...
    if (currentAction.tool === "pen" || currentAction.tool === "highlighter" || currentAction.tool === "eraser") {
      ...
      let end = pos;
      if (e.shiftKey) {
        const s = currentAction.start!; const dx = pos.x - s.x, dy = pos.y - s.y; const len = Math.max(Math.abs(dx), Math.abs(dy));
        const angle = Math.atan2(dy, dx); const snap = Math.round(angle / (Math.PI / 4)) * (Math.PI / 4);
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
    if (tool === "laser") { isLaserActiveRef.current = false; return; }
    if (isPanningRef.current) { isPanningRef.current = false; return; }
    if (isDraggingSelectedRef.current) { isDraggingSelectedRef.current = false; dragStartRef.current = null; setSaved(false); scheduleAutoSave(actions); return; }
    if (!isDrawing || !currentAction) return;
    setIsDrawing(false);
    const act = { ...currentAction };
    const hasContent = (act.points?.length ?? 0) > 0 || (act.start && act.end && (Math.abs(act.end.x - act.start.x) > 2 || Math.abs(act.end.y - act.start.y) > 2));
    if (hasContent) { setActions(prev => { const n = [...prev, act]; setSaved(false); scheduleAutoSave(n); return n; }); }
  ...
  const commitText = () => {
    ...
    const act: DrawAction = {
      ...
      text: textVal, start: { x: textOverlay.x, y: textOverlay.y },
      end: ft === "callout" ? { x: textOverlay.x + 260, y: textOverlay.y + 140 } : undefined,
    };
    setActions(prev => { const n = [...prev, act]; setSaved(false); scheduleAutoSave(n); return n; });
    setRedoStack([]); setTextOverlay(null); setTextVal("");
  };

  const handleImageFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file || !pendingImageRef.current) return;
    const reader = new FileReader();
    reader.onload = (evt) => {
      const imgData = evt.target?.result as string; if (!imgData || !pendingImageRef.current) return;
      const img = new Image();
      img.onload = () => {
        const aspect = img.width / img.height; const w = Math.min(400, img.width), h = w / aspect;
        const act: DrawAction = { id: uid(), tool: "image", color, fillColor: null, lineWidth: 0, opacity: 1, dashStyle: "solid", start: { ...pendingImageRef.current! }, end: { x: pendingImageRef.current!.x + w, y: pendingImageRef.current!.y + h }, imageData: imgData };
        setActions(prev => { const n = [...prev, act]; setSaved(false); scheduleAutoSave(n); return n; });
  ...
  useEffect(() => {
    const handlePaste = (e: ClipboardEvent) => {
      ...
      if (imgItem) {
        ...
        const reader = new FileReader();
        reader.onload = (evt) => {
          const imgData = evt.target?.result as string; if (!imgData) return;
          const img = new Image();
          img.onload = () => {
            const aspect = img.width / img.height; const w = Math.min(400, img.width), h = w / aspect;
            const pos = { x: 100 + Math.random() * 200, y: 100 + Math.random() * 200 };
            const act: DrawAction = { id: uid(), tool: "image", color, fillColor: null, lineWidth: 0, opacity: 1, dashStyle: "solid", start: pos, end: { x: pos.x + w, y: pos.y + h }, imageData: imgData };
            setActions(prev => { const n = [...prev, act]; setSaved(false); scheduleAutoSave(n); return n; });
  ...
  const handleWheel = (e: React.WheelEvent) => { if (e.ctrlKey || e.metaKey) { e.preventDefault(); setZoom(z => Math.min(5, Math.max(0.15, z - e.deltaY * 0.001))); } };
  const handleExport = (format: "png" | "jpeg" = "png") => {
    const canvas = canvasRef.current; if (!canvas) return;
    const link = document.createElement("a"); link.download = `whiteboard-${cid}-${Date.now()}.${format}`;
    link.href = format === "jpeg" ? canvas.toDataURL("image/jpeg", 0.92) : canvas.toDataURL("image/png"); link.click();
    toast({ title: `Exported as ${format.toUpperCase()}!` });
  };
  const bringToFront = () => { if (!selectedId) return; setActions(prev => { const i = prev.findIndex(a => a.id === selectedId); if (i < 0) return prev; const n = [...prev]; n.splice(i, 1); n.push(prev[i]); scheduleAutoSave(n); return n; }); };
  const sendToBack = () => { if (!selectedId) return; setActions(prev => { const i = prev.findIndex(a => a.id === selectedId); if (i < 0) return prev; const n = [...prev]; n.splice(i, 1); n.unshift(prev[i]); scheduleAutoSave(n); return n; }); };
  ...
  if (presentMode) {
    return (
      <div className="fixed inset-0 z-50 bg-black flex flex-col">
        <div className="absolute top-2 right-2 z-10"><button className="bg-white/20 hover:bg-white/30 text-white text-xs px-3 py-1.5 rounded-lg" onClick={() => setPresentMode(false)}>Exit</button></div>
        <div className="flex-1 overflow-auto"><canvas ref={canvasRef} width={CW} height={CH} className="w-full h-full object-contain" /></div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col h-screen select-none overflow-hidden ${isDark ? "bg-gray-950" : "bg-slate-100"}`} style={{ userSelect: "none" }}>
      ...
      <div className={`flex items-center gap-1.5 px-3 py-1.5 border-b flex-shrink-0 flex-wrap ${tb}`}>
        ...
        <span className={`text-xs font-semibold`}>Class Whiteboard</span>
        <div className={`h-4 w-px ${isDark ? "bg-gray-700" : "bg-gray-200"}`} />
        ...
        <button className={`${btnBase} h-7 w-7 ${toolInactive}`} onClick={() => setZoom(z => Math.max(0.15, +(z - 0.15).toFixed(2)))}><ZoomOut size={13} /></button>
        <button className={`text-xs font-mono w-10 text-center rounded px-1 py-0.5 border ${isDark ? "bg-gray-800 border-gray-600 text-gray-200" : "bg-gray-50 border-gray-200"}`} onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }}>{Math.round(zoom * 100)}%</button>
        <button className={`${btnBase} h-7 w-7 ${toolInactive}`} onClick={() => setZoom(z => Math.min(5, +(z + 0.15).toFixed(2)))}><ZoomIn size={13} /></button>
        <div className={`h-4 w-px ${isDark ? "bg-gray-700" : "bg-gray-200"}`} />
        ...
        <button onClick={() => setGridSnap(g => !g)} className={`${btnBase} h-7 px-2 gap-1 text-xs ${gridSnap ? "bg-emerald-600 text-white border-emerald-600" : toolInactive}`}><Grid size={12} />Snap</button>

        <div className="ml-auto flex items-center gap-1.5">
          <span className={`text-xs font-medium ${saved ? "text-green-500" : "text-amber-500"}`}>{saved ? "✓ Saved" : "● Unsaved"}</span>
          <button onClick={() => handleExport("png")} className={`${btnBase} h-7 px-2 gap-1 ${toolInactive}`}><Download size={12} />PNG</button>
          <button onClick={() => handleExport("jpeg")} className={`${btnBase} h-7 px-2 gap-1 ${toolInactive}`}><Download size={12} />JPG</button>
          ...
          <button className={`${btnBase} h-7 w-7 text-red-500 border-transparent ${isDark ? "hover:bg-red-900/30" : "hover:bg-red-50"}`} onClick={() => { if (!confirm("Clear whiteboard?")) return; setActions([]); setRedoStack([]); setSelectedId(null); setSaved(false); }}><Trash2 size={13} /></button>
          <button className={`${btnBase} h-7 px-3 gap-1 bg-primary text-white border-primary hover:bg-primary/90`} onClick={() => doSave()} disabled={saveWb.isPending}><Save size={12} />{saveWb.isPending ? "…" : "Save"}</button>
        </div>
      </div>
        ...
        <div className={`flex flex-col items-center gap-0.5 py-2 px-1 border-r flex-shrink-0 overflow-y-auto ${isDark ? "bg-gray-900 border-gray-700" : "bg-white border-gray-200"}`}>
          ...
          {selectedId && (
            <>
              ...
              <button onClick={bringToFront} title="Bring to front" className={`w-10 h-9 rounded-lg border text-xs ${toolInactive}`}><span style={{ fontSize: 11 }}>▲▲</span></button>
              <button onClick={sendToBack} title="Send to back" className={`w-10 h-9 rounded-lg border text-xs ${toolInactive}`}><span style={{ fontSize: 11 }}>▼▼</span></button>
              <button onClick={() => { setActions(prev => { const n = prev.filter(a => a.id !== selectedId); scheduleAutoSave(n); return n; }); setSelectedId(null); }}
                className={`w-10 h-9 rounded-lg border text-red-500 ${isDark ? "border-gray-600 hover:bg-red-900/30" : "border-gray-200 hover:bg-red-50"}`}><Trash2 size={13} /></button>
              <button onClick={() => {
                const sel = actions.find(a => a.id === selectedId);
                if (sel) { const newA = { ...sel, id: uid(), start: sel.start ? { x: sel.start.x + 20, y: sel.start.y + 20 } : undefined, end: sel.end ? { x: sel.end.x + 20, y: sel.end.y + 20 } : undefined, points: sel.points?.map(p => ({ x: p.x + 20, y: p.y + 20 })) }; setActions(prev => { const n = [...prev, newA]; scheduleAutoSave(n); return n; }); setSelectedId(newA.id); }
              }} className={`w-10 h-9 rounded-lg border ${toolInactive}`}><Copy size={13} /></button>
            </>
          )}
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          <div className={`flex items-center gap-2 px-3 py-1.5 border-b flex-shrink-0 flex-wrap overflow-x-auto ${tbSub}`}>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              {fillEnabled && (
                <label className="w-5 h-5 rounded border-2 border-gray-300 cursor-pointer overflow-hidden">
            ...
            {tool === "pen" && (
              <>
                <div className={`h-4 w-px shrink-0 ${isDark ? "bg-gray-700" : "bg-gray-200"}`} />
                <div className="flex items-center gap-0.5 shrink-0">
                  <span className={`text-xs ${isDark ? "text-gray-400" : "text-gray-500"}`}>Style:</span>
                  {([["round", "✏ Round"], ["marker", "▬ Marker"], ["calligraphy", "𝒞 Calli"], ["pencil", "✎ Pencil"]] as [PenStyle, string][]).map(([ps, label]) => (
                    <button key={ps} onClick={() => setPenStyle(ps)}
                      className={`text-xs px-1.5 py-0.5 rounded border transition-all ${penStyle === ps ? "bg-primary text-white border-primary" : isDark ? "border-gray-600 text-gray-300 hover:bg-gray-700" : "border-gray-200 hover:bg-gray-100"}`}>
                      {label}
                    </button>
                  ))}
                </div>
            ...
            {tool === "eraser" && (
              <>
                <div className={`h-4 w-px shrink-0 ${isDark ? "bg-gray-700" : "bg-gray-200"}`} />
                <div className="flex items-center gap-0.5 shrink-0">
                  <span className={`text-xs ${isDark ? "text-gray-400" : "text-gray-500"}`}>Mode:</span>
                  <button onClick={() => setEraserMode("stroke")}
                    className={`text-xs px-1.5 py-0.5 rounded border transition-all ${eraserMode === "stroke" ? "bg-primary text-white border-primary" : isDark ? "border-gray-600 text-gray-300 hover:bg-gray-700" : "border-gray-200 hover:bg-gray-100"}`}>
                    ⌫ Stroke
                  </button>
                  <button onClick={() => setEraserMode("element")}
                    className={`text-xs px-1.5 py-0.5 rounded border transition-all ${eraserMode === "element" ? "bg-primary text-white border-primary" : isDark ? "border-gray-600 text-gray-300 hover:bg-gray-700" : "border-gray-200 hover:bg-gray-100"}`}>
                    ✕ Element
                  </button>
                </div>
              </>
            ...
            {(tool === "text" || tool === "sticky" || tool === "callout") && (
              <>
                ...
                <select value={fontFamily} onChange={e => setFontFamily(e.target.value)} className={`text-xs rounded border px-1.5 py-0.5 ${isDark ? "bg-gray-800 border-gray-600 text-gray-200" : "bg-white border-gray-200"}`}>
                  {FONT_FAMILIES.map((f, i) => <option key={f} value={f}>{FONT_FAMILY_LABELS[i]}</option>)}
                </select>
                <button onClick={() => setFontBold(b => !b)} className={`text-xs px-1.5 py-0.5 rounded border font-bold ${fontBold ? "bg-primary text-white border-primary" : isDark ? "border-gray-600 text-gray-300" : "border-gray-200"}`}>B</button>
                <button onClick={() => setFontItalic(b => !b)} className={`text-xs px-1.5 py-0.5 rounded border italic ${fontItalic ? "bg-primary text-white border-primary" : isDark ? "border-gray-600 text-gray-300" : "border-gray-200"}`}>I</button>
              </>
            )}
            {tool === "sticky" && (
              <>
            ...
            <span className={`ml-auto text-[10px] whitespace-nowrap shrink-0 ${isDark ? "text-gray-700" : "text-gray-400"}`}>
              Ctrl+Z undo · Shift=lock · Space=pan · Ctrl+scroll=zoom
            </span>
          </div>
            ...
            <div style={{ width: CW * zoom + 80, height: CH * zoom + 80, display: "flex", alignItems: "center", justifyContent: "center", minWidth: "100%", minHeight: "100%", paddingTop: 40, paddingLeft: 40 }}>
              <div style={{ position: "relative", width: CW * zoom, height: CH * zoom, flexShrink: 0 }}>
                <canvas ref={canvasRef} width={CW} height={CH}
                  ...
                  onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp} data-testid="canvas-whiteboard" />
                <canvas ref={laserCanvasRef} width={CW} height={CH}
                ...
                {textOverlay && (
                  <div style={{ position: "absolute", left: textOverlay.cx, top: textOverlay.cy, zIndex: 20, pointerEvents: "auto" }}>
                    <textarea ref={textRef} value={textVal} onChange={e => setTextVal(e.target.value)} onBlur={commitText}
                      onKeyDown={e => { if (e.key === "Escape") { setTextOverlay(null); setTextVal(""); } if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) { e.preventDefault(); commitText(); } }}
                      rows={textOverlay.forTool === "sticky" ? 4 : 2}
                      placeholder={textOverlay.forTool === "sticky" ? "Sticky note…" : textOverlay.forTool === "callout" ? "Callout text…" : "Type text…"}
                      className="border-2 border-primary rounded-lg px-2 py-1 outline-none resize shadow-xl min-w-40"
                      style={{ fontSize: Math.max(11, fontSize * zoom), color, backgroundColor: isDark ? "#1e1e2e" : "#fff" }} />
                  </div>
                )}
              </div>
            </div>
