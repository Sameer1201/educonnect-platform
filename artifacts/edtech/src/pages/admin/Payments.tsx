...
import { CreditCard, CheckCircle, Clock, AlertTriangle, Bell, Plus, Filter, IndianRupee, Search, X, ChevronDown } from "lucide-react";
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <CreditCard size={26} className="text-indigo-600"/> Payment Management
          </h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage and track student tuition fees</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total", value: stats?.total ?? 0, color: "text-indigo-600", bg: "bg-indigo-50 dark:bg-indigo-900/20" },
          { label: "Paid", value: stats?.paid ?? 0, color: "text-green-600", bg: "bg-green-50 dark:bg-green-900/20" },
          { label: "Pending", value: stats?.pending ?? 0, color: "text-yellow-600", bg: "bg-yellow-50 dark:bg-yellow-900/20" },
          { label: "Overdue", value: stats?.overdue ?? 0, color: "text-red-600", bg: "bg-red-50 dark:bg-red-900/20" },
        ].map(s => (
          <div key={s.label} className={`${s.bg} rounded-xl p-4 border border-gray-100 dark:border-gray-700`}>
            <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{s.label}</div>
      ...
      {stats && (
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl p-5 text-white">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              <div className="text-indigo-200 text-sm mb-1">Total Collection</div>
              <div className="text-3xl font-bold">₹{(stats.collectedAmount ?? 0).toLocaleString("en-IN")}</div>
              <div className="text-indigo-200 text-sm mt-1">of ₹{(stats.totalAmount ?? 0).toLocaleString("en-IN")} total</div>
            </div>
            <div className="text-right">
              <div className="text-indigo-200 text-sm mb-1">Collection Rate</div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            {pendingCount > 0 && (
              <button onClick={() => sendRemindersMutation.mutate()}
                ...
                <Bell size={14}/> Remind Unpaid ({pendingCount})
              </button>
            )}
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2 flex gap-2">
                <button onClick={() => generateMutation.mutate()}
                  ...
                  {generateMutation.isPending ? "Generating…" : "Generate & Notify Students"}
                </button>
                <button onClick={() => { setShowGenModal(false); setGenMsg(""); }}
